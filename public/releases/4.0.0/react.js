...
function renderChildren(children, breaks, hangs, spaces, educate) {
  let offset = 0;
  ...
  const visit = (nodes) => Children.map(nodes, (child) => {
    if (typeof child === "string" || typeof child === "number") {
      ...
      const start = offset;
      offset += text.length;
      const stops = [.../* @__PURE__ */ new Set([...breaks, ...optical.keys(), ...spacing.keys()])].filter((at) => at >= start && at < offset).sort((a, b) => a - b);
      let cursor = 0;
      const pieces = [];
  ...
  componentDidUpdate(previous, _state, restoreSelection) {
    ...
    if (previous !== this.props) this.recompose();
      ...
      if (plan && (plan.outcome === "composed:rich" || plan.outcome === "native:fits")) {
        ...
        if (invalid) {
          this.setState({ plan: { ...plan, breaks: [], hangs: [], spacing: void 0, hanging: "native:hanging-verification", outcome: "native:verification" } });
          return;
        }
...
// src/lib/v4/typeset-react.tsx
function TypesetText({ text, as = "p", mode, keep, maxLines, density, lineBreaks, smartQuotes: smartQuotes2, opticalHanging, spacing, contour, ...attributes }) {
  const ref = useRef(null);
  const [initialText] = useState(text);
  const options = useRef({ text, mode, keep, maxLines, density, lineBreaks, smartQuotes: smartQuotes2, opticalHanging, spacing, contour });
  options.current = { text, mode, keep, maxLines, density, lineBreaks, smartQuotes: smartQuotes2, opticalHanging, spacing, contour };
  useLayoutEffect(() => {
    const element = ref.current;
    if (!element) return;
    const controller = mount(element, "[data-typeset-react]", options.current);
    return () => controller.disconnect();
  }, [as, text, mode, keep, maxLines, density, lineBreaks, smartQuotes2, opticalHanging, spacing, contour]);
  useLayoutEffect(() => {
    const element = ref.current;
