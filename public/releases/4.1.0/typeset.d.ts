...
export declare function isWeakEnding(word: string): boolean;
/**
...
declare function composeParagraph(tokens: Token[], measurePx: number, measureCh: number, opts?: ParagraphOptions): FrozenLine[] | null;
...
export declare function searchParagraph(problem: ParagraphProblem, limits?: {
    exactStates?: number;
    exactTokens?: number;
}): ParagraphSearchResult;
/** Internal final objective, also exercised independently by the oracle. */
export declare function chooseParagraphLayout(completes: ParagraphState[]): ParagraphState | null;
export declare function paragraphContour(widths: readonly number[]): number;
export declare function rankParagraphLayouts(completes: ParagraphState[], widths?: (lines: ParagraphLine[]) => number[]): {
    winner: ParagraphState | null;
    eligible: number;
    score: number | null;
};
declare function shapeExactLines(lines: FrozenLine[], measureCh: number, measurePx: number, isHeading?: boolean, spaceEm?: number, fontSizePx?: number): FrozenLine[] | null;
/**
 * Validate final composition before rendering.
 */
declare function finalValidate(lines: FrozenLine[], measureCh: number, isHeading?: boolean, spaceEm?: number, allowOrphan?: boolean): boolean;
/**
