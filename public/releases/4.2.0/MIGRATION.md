...
4.2.0 follows 4.1.0. No public APIs were renamed and no required runtime peer
was added. Retain the prior lockfile and deployment before upgrading.

## From 4.1.0

1. Disconnect the old controller/unmount its adapters before replacing the engine.
2. Install typeset.us@4.2.0, or use go@4.2.0.js with its hash from sri.json.
3. Keep title and prose selectors disjoint. A font-display paragraph is still a
   paragraph: exclude it from the prose selector, not from Typeset altogether.
4. Include venue/caption text explicitly when numeric wrappers previously hid
   it from the selector. Use title mode for short names and body mode for prose.
5. Test source preservation, focus/copying, updates, resizing and startup cost.
   Inspect controller.stats.overlappingTargets; it should be zero in a correct integration.

The same-instance overlap safeguard prevents competing controllers from repeatedly
rewriting a target. Inline nowrap phrases no longer exclude surrounding rich text.
English capitalized name/designator preferences can improve title and linked-list
breaks without changing authored content, font size or available width.

Rollback: restore the recorded dependency/deployment or typeset.us@4.1.0 and its
original go@4.1.0.js hash. https://typeset.us/releases/4.1.0/ stays immutable.

## Historical 4.0 to 4.1 migration
