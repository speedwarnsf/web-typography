...
await writeFile('public/release.json', JSON.stringify({ version, previous: '3.5.1', previousBrowserPin: '3.5.0', package: `typeset.us@${version}`, download: `/releases/${version}/${pack.filename}`, archive: '/releases/3.5.1/README.md', manifest: `/releases/${version}/manifest.json`, loader: { url: `/go@${version}.js`, integrity: sri(go), bytes: go.length, gzipBytes: gzipSync(go).length }, validation: 'See SUPPORT.md for verified coverage and outstanding device acceptance.' }, null, 2) + '\n');
