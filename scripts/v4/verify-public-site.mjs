...
const ready = page => page.evaluate(() => Promise.race([
  window.TypesetReady,
  new Promise((_, reject) => setTimeout(() => reject(new Error('Typeset readiness exceeded 15 seconds')), 15000)),
]));
  ...
  try {
    ...
    for (const width of [390, 1440]) {
      ...
      for (const path of ['/', '/proof', '/utility', '/perfect-paragraph', '/essay', '/pairing-cards']) {
        console.log(`${config.name} ${width} ${path}`);
    ...
    for (const path of ['/releases/3.5.1/', '/releases/4.0.0/', `/releases/${version}/`]) {
      console.log(`${config.name} archive ${path}`);
      const response = await page.goto(base + path);
    ...
    for (const explicit of [false, true]) {
      console.log(`${config.name} loader ${explicit}`);
      ...
      await page.evaluate(({ base, explicit, integrity, pin }) => new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error('Versioned loader exceeded 15 seconds')), 15000);
        ...
        script.onerror = () => { clearTimeout(timeout); reject(new Error('Versioned loader failed to load')); };
