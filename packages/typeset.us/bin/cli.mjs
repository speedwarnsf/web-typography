...
const PAGE_FN = ({ selector }) => {
  ...
  const els = Array.from(document.querySelectorAll(selector)).filter((el) => {
    ...
    return words.length >= 15 && el.getBoundingClientRect().width > 120;
  });
  if (!els.length) {
  ...
  return { graded: true, paragraphs, lines, violations, typesetPresent, composed };
};
...
try {
  ...
  if (asJson) {
    ...
    console.log(`${result.paragraphs} paragraphs, ${result.lines} lines measured`);
    for (const v of result.violations.slice(0, 25)) {
      console.log(`  ${v.type}: "${v.word}" — ${v.paragraph}…`);
    }
    if (n > 25) console.log(`  …and ${n - 25} more`);
    if (result.composed) console.log(`typeset audit(): ${result.composed.length} violations in composed content`);
    console.log(n === 0 && (result.composed?.length ?? 0) === 0
      ? 'clean: 0 violations'
      : `${n + (result.composed?.length ?? 0)} violations — one script tag fixes this: https://typeset.us`);
  }
  const dirty = (result.graded && result.violations.length > 0) || (result.composed?.length ?? 0) > 0;
